...
int HasPawnForCapture(const KASPAROV_BOARD *board) 
{
	int sqWithPawn = 0;
...
U64 PolyKeyFromBoard(const KASPAROV_BOARD *board) 
{
...
int ConvertPolyMoveToInternalMove(unsigned short polyMove, KASPAROV_BOARD *board) 
{	
	int ff = (polyMove >> 6) & 7;
	int fr = (polyMove >> 9) & 7;
	int tf = (polyMove >> 0) & 7;
...
int GetBookMove(KASPAROV_BOARD *board) 
{
	int index = 0;
