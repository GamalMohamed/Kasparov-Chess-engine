...
int MaterialDraw(const KASPAROV_BOARD *pos)
{
	...
	else if (!pos->pceNum[wQ] && !pos->pceNum[bQ]) 
	{
		...
		{
			if ((pos->pceNum[wN] + pos->pceNum[wB] == 0) && (((pos->pceNum[bN] + pos->pceNum[bB]) == 1) ||
				((pos->pceNum[bN] + pos->pceNum[bB]) == 2))) { return TRUE; }
		}
		else if (pos->pceNum[bR] == 1 && !pos->pceNum[wR]) 
		{
			if ((pos->pceNum[bN] + pos->pceNum[bB] == 0) && (((pos->pceNum[wN] + pos->pceNum[wB]) == 1) 
...
int EvalPosition(const KASPAROV_BOARD *pos) 
{
	ASSERT(CheckBoard(pos));
