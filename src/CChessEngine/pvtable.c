...
int GetPvLine(const int depth, KASPAROV_BOARD *pos)
{
...
void ClearHashTable(SEARCH_HASHTABLE *table)
{
	SEARCH_HASHENTRY *tableEntry;
...
void InitHashTable(SEARCH_HASHTABLE *table, const int MB)
{
	int HashSize = 0x100000 * MB;
	table->numEntries = HashSize / sizeof(SEARCH_HASHENTRY);
	table->numEntries -= 2;
	...
	table->pTable = (SEARCH_HASHENTRY *)malloc(table->numEntries * sizeof(SEARCH_HASHENTRY));
	if (table->pTable == NULL)
	{
...
int ProbeHashEntry(KASPAROV_BOARD *pos, int *move, int *score, int alpha, int beta, int depth)
{
...
void StoreHashEntry(KASPAROV_BOARD *pos, const int move, int score, const int flags, const int depth) 
{
	int index = pos->posKey % pos->HashTable->numEntries;
...
int ProbePvMove(const KASPAROV_BOARD *pos) 
{
	int index = pos->posKey % pos->HashTable->numEntries;
	ASSERT(index >= 0 && index <= pos->HashTable->numEntries - 1);
